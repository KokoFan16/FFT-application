...
//
//	  if (rank == 0)
//		  printf("%d: %s\n", rank, "transpose-pairwise I->O, O->I");
//     }
//     else {
//	  /* transpose chunks globally */
//	  transpose_chunks(ego->sched, ego->n_pes, ego->my_pe,
...
//     /* transpose locally to get contiguous chunks */
     ...
     if (cld1) {
	  ...
	  if (ego->equal_blocks)
	  {
		   ...
		   uniform_inverse_isplit_r_bruck(64, MPI_IN_PLACE, ego->send_block_sizes[0], FFTW_MPI_TYPE,
...
//	    	   printf("%s, %d\n", "3--MPI_Alltoall", (I == O));
	       ...
	       MPI_Alltoall(MPI_IN_PLACE, ego->send_block_sizes[0], FFTW_MPI_TYPE,
			    O, ego->recv_block_sizes[0], FFTW_MPI_TYPE,
			    ego->comm);
//	    	   printf("%d: %s, %d, %d\n", rank, "3--MPI_Alltoall", ego->send_block_sizes[0], ego->recv_block_sizes[0]);
...
//				 (!ego->copy_transposed_in), (p->flags & TRANSPOSED_IN),
//				 (!ego->copy_transposed_in), (ONLY_TRANSPOSEDP(p->flags))
//				 );
     return 1;
//     return (1
//	     && p->I != p->O
...
//			 if (my_pe == 0 )
//				 printf("1--MKPLAN_MPI_TRANSPOSE\n");
	  }
	  else {
	       I = p->O; /* final transpose is in-place */

//			 if (my_pe == 0 )
//				 printf("2--MKPLAN_MPI_TRANSPOSE\n");
	  }
     }
